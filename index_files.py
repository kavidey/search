...
from database import setup_database, create_session, File, Word, add_file_with_words, file_indexed
        ...
        for entry in entries:
            if entry.is_file:
                ...
                if not file_indexed(session, entry.stem, str(entry.parent)):
                    ...
                    f = File(filename=entry.stem, content='', path=str(entry.parent), filetype=entry.suffix)
