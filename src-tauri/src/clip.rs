...
impl ClipModel {
    pub fn new(model_path: &str, tokenizer_path: &str, use_cpu: bool) -> anyhow::Result<Self> {
        ...
        let config = clip::ClipConfig::vit_base_patch32();
        let vb = unsafe {
            VarBuilder::from_mmaped_safetensors(&[model_path.to_string()], DType::F32, &device)?
        };
        let model = clip::ClipModel::new(vb, &config)?;
        let tokenizer = get_tokenizer(Some(tokenizer_path.to_string()))?;
    ...
    pub fn process_images_and_text(
        ...
        let images = load_images(&image_paths, self.model.config().image_size)?
            .to_device(&self.device)?;
        let (input_ids, vec_seq) =
            tokenize_sequences(Some(text_sequences), &self.tokenizer, &self.device)?;
        let (_logits_per_text, logits_per_image) = self.model.forward(&images, &input_ids)?;
        let softmax_image = softmax(&logits_per_image, 1)?;
        let softmax_image_vec = softmax_image.flatten_all()?.to_vec1::<f32>()?;

        let probability_vec: Vec<f32> = softmax_image_vec.iter().map(|v| v * 100.0).collect();
        ...
        for i in 0..image_paths.len() {
            ...
            let end = start + probability_per_image;
            let prob = &probability_vec[start..end];
            let image_results: Vec<(String, f32)> = prob
                .iter()
                .enumerate()
                .map(|(j, &p)| (vec_seq[j].clone(), p))
                .collect();
